...
/**
 * Yield all updates on the cell's upstream signals that the cell hasn't
 * handled yet. Discovers the cell's upstream signals via
 * `graph.getCellInputs(cellId)`; the per-URI watermark is keyed on the
 * `cellId` itself (the cell's `handled` rows), NOT on any output signal —
 * so a cell needs no output to track its progress.
 *
 * Opens one URI-ordered diff stream per upstream signal (via
 * `readUpdates({ cell: cellId, orderBy: "uri" })`) and merges them with a
 ...
 * upstream signals, the entries are emitted adjacently in the order the
 * upstream signals were declared in `graph.getCellInputs(cellId)`, so
 * consumers can collapse per-URI in one forward pass without buffering.
 *
 * A URI updated by N upstream signals appears N times (once per upstream
 * entry). Use `aggregateByUri` if the consumer wants one record per URI.
 *
 * Cells with no inputs (probers) yield nothing. Sink cells (inputs but no
 * outputs) work normally — the cell-id watermark removes the old
 * "first output signal" requirement.
 */
export async function* readCellUpdates(
  store: UpdatesStore,
  graph: DataflowGraph,
  cellId: CellId,
  ...
  const iters: Array<AsyncIterator<UpdateEntry>> = inputs.map((upstream) => {
    const iterable = store.readUpdates({
      signal: upstream,
      cell: cellId,
      uriPrefix,
      orderBy: "uri",
...
/**
 * Drain a cell-updates stream into a Map keyed by URI, where each value is
 * the list of upstream entries that contributed (potentially one per
 * upstream signal). Useful when the cell's per-URI work depends on which
 * upstream signal(s) are fresh, or simply to dedupe URIs when the cell only
 * cares that "something" changed.
 *
 * Insertion order of the returned Map preserves encounter order in the
 * source stream (so within each upstream signal, URIs are URI-ascending;
 * across upstream signals, the order follows `graph.getCellInputs`).
 */
